-- Keep the per-file upload ceiling consistent across Storage, reservations,
-- published assets, and both legacy and batched reservation functions.
...
alter table public.assets
  ...
  check (
    size > 0
    and (
      (not is_legacy and size <= 209715200)
...
begin
  ...
  if p_path not like (p_space_id::text || '/%')
    or p_size < 1 or p_size > 209715200 then
    return false;
  end if;
...
begin
  ...
  if exists (
    ...
    where coalesce(file->>'path', '') not like (p_space_id::text || '/%')
      ...
      or coalesce((file->>'size')::bigint, 0) > 209715200
  ) then
    return false;
  end if;
...
begin
  ...
  loop
    if item_record->>'path' not like (p_space_id::text || '/%')
      ...
      or coalesce((item_record->>'size')::bigint, 0) > 209715200 then
